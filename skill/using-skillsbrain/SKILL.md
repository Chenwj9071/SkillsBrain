...
description: SkillsBrain 的“任务前置入口”。在执行任何本地任务（开发/测试/运行/排障/脚本/工具安装）前，优先先用 `skillsbrain status/serve/match/list` 确认服务与可用技能，再决定后续操作；也用于管理订阅源与索引维护。
---
...
2. 需要在本机执行任何命令或脚本（包括 `pip install`、`npm`、`python -m ...`、`git`、编译、测试等）。
